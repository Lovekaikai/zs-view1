                  ...
                  <div class="ordercon-five">
                    <div>
                      <div v-if="Number(order.status)===1">
                        <el-button type="text" @click="confirmOrder(order.orderNo)">确认订单{{$t('button.confirmOrder')}}</el-button>
                      </div>
                      <div v-if="Number(order.status)===1">
                        <el-button type="text" @click="cancelOrder(order.orderNo,1)">取消订单{{$t('button.cancelOrder')}}</el-button>
                      </div>
                      <div v-if="Number(order.status)===2">
                        <el-button type="text" @click="cancelOrder(order.orderNo,1)">商品退款{{$t('button.refund')}}</el-button>
                      </div>
                      <div v-if="Number(order.status)===2">
                        <el-button type="text" @click="godelivery(order.orderNo)">立即发货{{$t('button.delivery')}}</el-button>
                      </div>
                      <div v-if="Number(order.status)===1 || 2 || 3|| 4|| 5|| 6|| 7 || 8">
                        <el-button type="text" @click="goOrderDetail(order.orderNo)">查看详情{{$t('button.viewDetail')}}</el-button>
                      </div>
                    </div>
                  </div>
                </div>
  ...
  methods: {
    ...
    exportOrder () {
      this.$confirm(this.$t('order.orderList.exportOrderTitle'), this.$t('confirm.tip'), {
        ...
        // 'http://zs-beta.cntracechain.com/back-end/merchantOrder/export?orderNo='
        ...
        window.location.href =
          'http://zs-beta.cntracechain.com/back-end/merchantOrder/export?merchantId=' + this.merchantId + '&orderNo=' +
